...
pub(crate) struct Allocation<'ctx, 'tcx> {
    ...
    /// Parent allocation for sub-allocations created by `split_at` /
    /// `from_raw_parts` / `as_chunks`. The sub-view has its own fresh `base`;
    /// the address linkage back to the parent lives in the pointer term
    /// (`parent_term + offset`) and provenance `offset`, not in base arithmetic.
    /// `root_alloc` follows this edge.
    pub parent: Option<AllocId>,

    /// Data allocation behind a fat pointer or heap-backed container: the slice
    /// contents for a `&[T]` reference, or the heap buffer for `Vec`/`String`/
    /// `CString`. Followed by `as_ptr()`/`as_mut_ptr()` redirection and by
    /// `drop` to cascade the `dead` flag.
